...
const getAvatarHint = (customer: Customer): string => {
  if (customer.profilePictureUrl && 
      ...
      !customer.profilePictureUrl.startsWith('https://picsum.photos') && // picsum is also a placeholder
      !customer.profilePictureUrl.startsWith('https://placehold.co')) { // placehold.co is also a placeholder
...
export default async function CustomerProfilePage({ params }: CustomerProfilePageProps) {
  let customer: Customer | undefined = mockCustomers.find((c) => c.id === params.id); 
  ...
  if (!customer) {
    try {
        ...
        const customersFromApi: Customer[] = await res.json();
        const foundCustomerFromApi = customersFromApi.find(c => c.id === params.id);
        
        if (foundCustomerFromApi) {
            customer = foundCustomerFromApi; // Use the customer found from the API
        } else {
            // If not found in mock and not found in API, then call notFound
            return notFound(); 
        }
    } catch (error) {
  ...
  // If customer is still not defined here, it means it truly wasn't found.
  if (!customer) {
    return notFound();
  }

  // Ensure customerBookings uses the final 'customer' object, which might be from API
