...
async function getCustomerBookings(customerId: string): Promise<Booking[]> {
    const res = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:9002'}/api/bookings?customerId=${customerId}`, { cache: 'no-store' });
    if (!res.ok) {
        console.error(`Failed to fetch bookings for customer ${customerId}: ${res.status} ${res.statusText}`);
        // Return empty array on error to allow page to render
        return [];
    }
    let bookings: Booking[] = await res.json();
    
    // Sort bookings by createdAt descending, as the API no longer guarantees this order when filtered by customerId
    bookings.sort((a, b) => {
      const dateA = a.createdAt && isValid(parseISO(a.createdAt)) ? parseISO(a.createdAt) : new Date(0);
      const dateB = b.createdAt && isValid(parseISO(b.createdAt)) ? parseISO(b.createdAt) : new Date(0);
      
      if (dateA.getTime() === new Date(0).getTime() && dateB.getTime() === new Date(0).getTime()) return 0; // both invalid or missing
      if (dateA.getTime() === new Date(0).getTime()) return 1; // a is invalid/missing, put last
      if (dateB.getTime() === new Date(0).getTime()) return -1; // b is invalid/missing, put last
    ...
    return bookings.map(booking => ({
      ...booking,
      // Ensure startTime and endTime are consistently available or handled
      startTime: booking.startTime || "N/A", 
      endTime: booking.endTime || "N/A",
      bookingDate: booking.bookingDate, // Assuming bookingDate is already in "yyyy-MM-dd" string format
    }));
}
...
export default async function CustomerProfilePage({ params }: CustomerProfilePageProps) {
  ...
  // Determine avatar URL based on gender if profilePictureUrl is a placeholder
  let avatarUrl = customer.profilePictureUrl || defaultUnknownAvatarUrl;
  if (avatarUrl === defaultUnknownAvatarUrl || avatarUrl.startsWith('https://placehold.co') || avatarUrl.startsWith('https://picsum.photos')) {
    switch (customer.gender) {
        case "male":
            avatarUrl = defaultMaleAvatarUrl;
            break;
        case "female":
            avatarUrl = defaultFemaleAvatarUrl;
            break;
        default:
            avatarUrl = defaultUnknownAvatarUrl;
            break;
    }
  }
        ...
        <Card className="md:col-span-1">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-foreground">Joined: {customer.createdAt && isValid(parseISO(customer.createdAt)) ? format(parseISO(customer.createdAt), "MMMM d, yyyy") : 'N/A'}</span>
            </div>
          </CardContent>
        </Card>
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="font-semibold text-foreground">
                           {booking.bookingDate && isValid(parseISO(booking.bookingDate)) ? format(parseISO(booking.bookingDate), "MMMM d, yyyy") : 'Date N/A'}
                        </p>
